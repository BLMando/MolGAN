...
if __name__ == "__main__":
    ...
    # Example 1: From CSV
    print("=" * 60)
    print("Example: Preprocessing event log from CSV")
    print("=" * 60)
    
    # Create example CSV data
    example_data = {
        'case:concept:name': [1, 1, 1, 2, 2, 3, 3, 3, 3],
        'concept:name': ['A', 'B', 'C', 'A', 'C', 'A', 'B', 'D', 'C'],
    ...
    # Extract traces
    print("\n1. Extracting traces...")
    traces, metadata = extract_traces(df, min_length=2)
    ...
    traces = add_start_end_tokens(traces)
    for i, trace in enumerate(traces[:3]):
        print(f"  Trace {i+1}: {' -> '.join(trace)}")
    
    # Build vocabulary
    print("\n3. Building vocabulary...")
    activity_to_idx, idx_to_activity, activity_counts = build_vocabulary(traces)
    print(f"Vocabulary size: {len(activity_to_idx)}")
    print(f"Activities: {list(activity_to_idx.keys())}")
    
    # Compute statistics
    print("\n4. Computing statistics...")
    stats = compute_trace_statistics(traces)
    print(f"Number of traces: {stats['num_traces']}")
    print(f"Unique activities: {stats['num_unique_activities']}")
    print(f"Avg trace length: {stats['avg_trace_length']:.2f}")
    print(f"Most common activities: {stats['most_common_activities'][:5]}")
    
    # Build DFG
    print("\n5. Building Directly-Follows Graph...")
    dfg = build_dfg(traces)
    print(f"DFG has {len(dfg)} edges")
    print("Top 5 edges:")
    sorted_edges = sorted(dfg.items(), key=lambda x: x[1], reverse=True)
    for (act_i, act_j), freq in sorted_edges[:5]:
        print(f"  {act_i} -> {act_j}: {freq}")
    
    # Split data
    print("\n6. Splitting into train/val/test...")
    # Add more traces for splitting example
    traces = traces * 10
    splits = split_traces(traces, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15)
    print(f"Train: {len(splits['train'])} traces")
    print(f"Val: {len(splits['val'])} traces")
    print(f"Test: {len(splits['test'])} traces")
    
    # Save preprocessed data
    print("\n7. Saving preprocessed data...")
    save_preprocessed_data(
        'preprocessed_traces.pkl',
        splits['train'],
        activity_to_idx,
        idx_to_activity
    )
